...
class Student(Human):
    ...
    def __eq__(self, other):
        """Додано для порівняння студентів за прізвищем під час пошуку у множині (set)"""
        if isinstance(other, Student):
            return self.last_name == other.last_name
        return False

    def __hash__(self):
        """Додано для можливості додавання об'єктів Student до set"""
...
class Group:
    ...
    def __str__(self):
        all_students = '\n '.join(str(student) for student in self.group) if self.group else 'Немає студентів'
...
gr.delete_student('Taylor')  
print("Видалення пройшло успішно, програма працює без помилок!")
